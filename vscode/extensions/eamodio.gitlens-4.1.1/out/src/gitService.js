...
class GitService extends vscode_1.Disposable {
    ...
    _getLogForFile(repoPath, fileName, sha, range, maxCount, reverse, entry, key) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const log = git_1.GitLogParser.parse(data, 'file', root, file, sha, maxCount, reverse, range);
                return log;
            }
            catch (ex) {
                if (entry) {
    ...
    static _toReferenceGitContentUri(commit, scheme, commitCount, data, dateFormat) {
        ...
        return vscode_1.Uri.parse(`${scheme}:${pad(data.index || 0)} \u2022 ${encodeURIComponent(message)} \u2022 ${moment(commit.date).format(dateFormat)} \u2022 ${encodeURIComponent(uriPath)}?${JSON.stringify(data)}`);
    }
